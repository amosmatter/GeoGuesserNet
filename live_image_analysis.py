...
from eval_functions import INVERTED_CATEGORIES
...
class LiveImageApp(QMainWindow):
    def __init__(self, root, modell, categories, transforms):
        ...
        self.timer.timeout.connect(
            lambda: asyncio.ensure_future(self.on_image_selected())
        )
    ...
    async def on_image_selected(self):
        ...
        hscale = 1
        rect = (
            width // 2 - width * hscale / 2,
            height // 2 - height * vscale / 2,
            width // 2 + width * hscale / 2,
            height // 2 + height * vscale / 2,
        )
        ...
        self.image_label.setPixmap(
            processed_pixmap.scaled(384, 384, Qt.KeepAspectRatio)
        )

        result_coarse = {}
        for city, region in INVERTED_CATEGORIES.items():
            result_coarse.setdefault(region, 0)
            result_coarse[region] += scores[city]

        self.display_results(
            result_coarse, self.rough_category_layout, self.store_rough_dict
        )
